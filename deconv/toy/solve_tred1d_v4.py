...
def solve_lasso(A, y, lam=1E-5):
    pipe = make_pipeline(Lasso(alpha=lam, positive=True))
    pipe.fit(A, y)
    return pipe.named_steps['lasso'].coef_
...
def solve_one_group(fr, hqs_group, pt0_group, pt1_group, spacing, lam0=0.0, lam1=0.1, end_exclusive=True):
    ...
    q_hat = solve_lasso(A, y, lam=lam0)
...
def main():
    ...
    plt.title("Charge sum q_hat vs effq per pixel; lambda=%.1e" % args.lam0)
    ...
    plt.title("Recorded hit charge sum vs effq per pixel; lambda=%.1e" % args.lam0)
