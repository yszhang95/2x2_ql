...
def load_source_data(source_hdf5):
    ...
    with h5py.File(source_hdf5, 'r') as f:
        ...
        points = f['picked/points'][:]
        event_ids = f['picked/event_id'][:]
    ...
    for i in range(len(event_ids)):
        sel = selected[selected['event_id'] == event_ids[i]]
        desel = deselected[deselected['event_id'] == event_ids[i]]
        selected_data.append(sel)
        deselected_data.append(desel)
    return p_min, p_max, event_ids, selected_data, deselected_data
...
def main():
    ...
    for ie in range(len(event_ids)):
        ...
        # transform a string like "a_event_id{}_b.hdf5"
        # to "a_event_id123_b.hdf5"
        pgun_hdf5 = args.pgun_hdf5.format(event_ids[ie])
        with h5py.File(pgun_hdf5, 'r') as fpgun:
            points = fpgun[args.dtype][:]
            pts = split_events(points)
    ...
    with h5py.File(args.out_hdf5, 'w') as fout:
        fout.create_dataset('hits/selected/data',
                            data=np.concatenate(sel_pts))
        fout.create_dataset('hits/deselected/data',
